...
def image_augmentor(image, input_shape, data_format, output_shape, zoom_size=None,
    ...
    :param keep_aspect_ratios: True, False      #纵横比
    :param constant_values:
    :param color_jitter_prob: prob of color_jitter      #彩色抖动的可能性
    :param rotate: [prob, min_angle, max_angle]     #prob：可能性
    ...
    if data_format not in ['channels_first', 'channels_last']:
        raise Exception("data_format must in ['channels_first', 'channels_last']!")     #主动抛出异常
    ...
    if ground_truth is not None:
        ymin = tf.reshape(ground_truth[:, 0], [-1, 1])  #列数为1, 行数除得
    ...
    if data_format == 'channels_first':
        image = tf.transpose(image, [1, 2, 0])      #改为channel_last
    ...
    if keep_aspect_ratios:      #是否保持纵横比缩放
        if fill_mode in ['NEAREST_NEIGHBOR', 'BILINEAR', 'BICUBIC']:
            zoom_ratio = tf.cond(              #若条件成立则返回f1,否则返回f2
    ...
    if color_jitter_prob is not None:
        ...
        image = tf.cond(bcs[0] < color_jitter_prob,
                        lambda: tf.image.adjust_brightness(image, tf.random_uniform([], 0., 0.3)),  #亮度
                        lambda: image
                )
        image = tf.cond(bcs[1] < color_jitter_prob,
                        lambda: tf.image.adjust_contrast(image, tf.random_uniform([], 0.8, 1.2)),   #对比度
                        lambda: image
                )
        image = tf.cond(bcs[2] < color_jitter_prob,
                        lambda: tf.image.adjust_hue(image, tf.random_uniform([], -0.1, 0.1)),   #色调
                        lambda: image
                )
    ...
    if ground_truth is not None:
        ...
        ymin = tf.boolean_mask(ymin, mask)      #保留满足mask的值
        ...
        ymin = tf.where(ymin < 0., ymin - ymin, ymin)   #满足则返回前者
