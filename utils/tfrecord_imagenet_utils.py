...
class ImageReader(object):  #读取图片并解码
        def __init__(self):
            self._decode_jpeg_data = tf.placeholder(dtype=tf.string)    #输入图像定义为string类型
...
def int64_feature(values):
    if not isinstance(values, (tuple, list)):
        values = [values]
    return tf.train.Feature(int64_list=tf.train.Int64List(value=values))    #生成训练的数据
...
def dataset2tfrecord(img_dir, output_dir, name, total_shards=50):   #/img_dir/class_dir/images
    ...
    random.shuffle(imglist)     #打乱图片的顺序
    num_per_shard = int(math.ceil(len(imglist)) / float(total_shards))      #分块
    for shard_id in range(total_shards):
        ...
        with tf.python_io.TFRecordWriter(outputname) as tf_writer:  #导出TFRecords
            ...
            for i in range(start_ndx, end_ndx):
                ...
                example = tf.train.Example(features=tf.train.Features(      #represents a {"string": value} mapping
                                           feature=features))
